...
The accompanying article, *Diffusion MRI toolkits differ in how they weight a tensor fit, not in how they implement it*, compares tensor estimators across the three toolkits. Every one of its tables is regenerated by a script in `scripts/`. Run the scripts inside the container, from the repository root, in the order listed in Section S6 of [`supplementary_jnm.md`](supplementary_jnm.md). Each writes a plain-text results file to the repository root. FSL is needed for the FSL arms, so use the Docker image if FSL is not installed locally.

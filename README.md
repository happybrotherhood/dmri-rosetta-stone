...
> The container bundles FSL 6.0.7, MRtrix3 3.0.8 and DIPY 1.12.1. No separate installation needed.
...
The accompanying article, *Diffusion MRI toolkits differ in which estimator they use, not in how they implement the tensor fit*, compares tensor estimators across the three toolkits. Every one of its tables is regenerated by a script in `scripts/`. Run the scripts inside the container, from the repository root, in the order listed in Section S6 of [`supplementary_jnm.md`](supplementary_jnm.md). Each writes a plain-text results file to the repository root. FSL is needed for the FSL arms, so use the Docker image if FSL is not installed locally.
...
If you use this tool in teaching or research, please cite the archived release:

```bibtex
@software{mutlu_dmri_rosetta_stone,
  title  = {dMRI Rosetta Stone: a controlled comparison of diffusion tensor estimators across FSL, MRtrix3 and DIPY},
  author = {Mutlu, Busra},
  year   = {2026},
  doi    = {10.5281/zenodo.22106454},
  url    = {https://github.com/happybrotherhood/dmri-rosetta-stone}
}
```

The DOI is the Zenodo concept DOI, which always resolves to the latest release.

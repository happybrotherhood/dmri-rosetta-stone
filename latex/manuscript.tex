...
\begin{figure}[tbp]
  ...
  \caption{Design of the inter-tool comparison. Both datasets are open and require no credentials. \emph{Left:} brain extraction is compared with each tool run on the input its algorithm is designed to consume --- \texttt{bet} on the mean b = 0 image, \texttt{dwi2mask} on the full DWI series, \texttt{median\_otsu} on the full series with the b = 0 volumes indexed --- so the Dice coefficients reflect the difference a user would encounter in practice. \emph{Right:} tensor fitting is compared with every tool given identical input: one brain mask, one volume subset comprising b = 0 and a single non-zero shell, and no preprocessing. Holding these constant isolates the fitting stage, which is what the metric comparison is intended to measure. Statistics are then restricted to physically admissible voxels, without which a few hundred failed fits dominate the correlation. Toolkit colours are consistent across Figures 1, 3 and 4. The diagram is generated by \texttt{scripts/make\_fig1\_design.py}.}
  \label{fig:Figure1}
\end{figure}
...
The ordering is the same on both datasets: FSL produces the most non-physical fits, MRtrix3 roughly half as many, and DIPY none. DIPY's \texttt{TensorModel} did not return a single voxel with FA \textgreater{} 1 or MD $\leq$ 0 in either acquisition (20 and 3 voxels respectively exceeded the free-water diffusivity bound). What changes between datasets is the rate, not the ranking: Sherbrooke, which has one b = 0 volume against Stanford's ten, produced roughly ten times as many failed fits in both toolkits that allow them.
...
Runtimes below are indicative rather than benchmarked, and were observed on one machine (macOS 14, Apple Silicon, 16 GB RAM; Docker Desktop with 8 GB allocated). Because the image is built for \texttt{linux/amd64}, it runs under emulation on this host, so a native x86-64 machine should be faster.

On the full 160-volume Stanford HARDI dataset, every stage except eddy correction completes in a few minutes. Eddy correction with FSL \texttt{eddy\_cpu} is the one bottleneck, taking on the order of 35--45 minutes on a single core; the demo subsample mode (20 volumes) brings this down to a few minutes while preserving the educational content of the stage. The practical implication for teaching is that only one of the seven stages needs to be planned around, and the platform provides a shortened path through it.
...
Containerised deployment is especially valuable in workshop contexts. Organisers can distribute the Docker image to participants in advance, eliminating the dependency installation bottlenecks that typically consume significant hands-on session time. The application runs on standard laptop hardware without GPU acceleration. Development and testing were carried out on macOS 14 (Apple Silicon) with Docker Desktop. The image is built for \texttt{linux/amd64} and therefore runs on Linux and Windows hosts as well, but we have not tested it there, and on Apple Silicon it runs under emulation, which slows execution.

...
4. **Which scheme is accurate?** Phantoms with known eigenvalues and realistic Rician noise, at three SNR levels, answer this directly (Table 6). Simulations on both real gradient tables show that the MD result holds on each protocol, while the FA result depends on the protocol (Supplementary Table S3).
5. **Could a few failed fits distort the statistics?** They would, so they were excluded. All statistics use physically admissible voxels only, and the number of failed fits is reported (Table 9).
6. **Does noise explain the real-data differences?** Largely. Noise added to each voxel's own fitted tensor, at the noise level measured on the Stanford b = 0 volumes, reproduces 89–106% of the FA offsets and 80–91% of the MD offsets, depending on how non-physical fits are treated (Table 7; Supplementary Table S6). It falls short in Stanford's most anisotropic voxels, where the residuals show the pattern expected of single-tensor misfit. We found no gradient-table error or signal dropout, and we report the remaining offsets as a limitation.

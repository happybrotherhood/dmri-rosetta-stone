...
These per-tool means are computed over each tool's own physically admissible voxels, whereas the Bland–Altman bias in Table 3 is computed over voxels admissible in *both* tools of a pair. The two therefore need not agree exactly, and on Sherbrooke — where the number of excluded voxels differs substantially between toolkits — they do not: the FSL-minus-MRtrix3 difference in means is +0.0077 FA units against a paired bias of +0.0139. The paired statistic is the appropriate one for agreement, since it compares the same voxels in both tools.
...
Runtimes below are indicative rather than benchmarked, and were observed on one machine (macOS 14, Apple Silicon, 16 GB RAM; Docker Desktop with 8 GB allocated). Because the image is built for `linux/amd64`, it runs under emulation on this host, so a native x86-64 machine should be faster.

On the full 160-volume Stanford HARDI dataset, every stage except eddy correction completes in a few minutes. Eddy correction with FSL `eddy_cpu` is the one bottleneck, taking on the order of 35–45 minutes on a single core; the demo subsample mode (20 volumes) brings this down to a few minutes while preserving the educational content of the stage. The practical implication for teaching is that only one of the seven stages needs to be planned around, and the platform provides a shortened path through it.
...
The resources in Table 2 were built for different purposes, and the comparison is not a ranking: each is stronger than dMRI Rosetta Stone at what it was designed for. The FSL Course and the MRtrix3 documentation are far more complete on their respective toolkits than anything here. What Table 2 shows is that the particular combination we set out to provide — three toolkits, executable, on the same real data, without a local neuroimaging installation — is not currently available elsewhere.

Our own main cost is that a Docker installation and the project image are required (see Limitations for its size) rather than the platform being fully browser-native. Docker is currently necessary to bundle FSL and MRtrix3, which do not support WebAssembly compilation. Future work may remove this barrier.
...
**Figure 1.** Software architecture of the dMRI Rosetta Stone Docker container. *Build time (top):* a two-stage build copies MRtrix3 3.0.4 binaries from the official `mrtrix3/mrtrix3` image into an `ubuntu:22.04` stage that installs FSL 6.0.7 via `fslinstaller.py` and the Python stack (Streamlit, DIPY, nibabel, numpy, scipy, matplotlib) via pip. Separating the two avoids the dependency conflicts that arise when FSL and MRtrix3 are installed into a single stage. *Run time (bottom):* the user reaches the Streamlit application through a browser on port 8501, and the application dispatches pipeline commands to all three toolkits via Python `subprocess` inside the container. Open sample data is mounted from the host. No neuroimaging software is installed on the host machine. The diagram is generated by `scripts/make_fig1_architecture.py`.
